...
- (void)setupCenterButton {
    ...
    self.centerButton.backgroundColor = [[UIColor alloc] initWithRed:84/255.0 green:214/255.0 blue:190/255.0 alpha:1.0];
...
- (void)setupExtraTabBarItems {
    ...
    self.extraLeftButton.tintColor = [UIColor whiteColor];
    ...
    self.extraRightButton.tintColor = [UIColor whiteColor];
    [self.extraRightButton addTarget:self action:@selector(didPressExtraRightButton:) forControlEvents:UIControlEventTouchUpInside|UIControlEventTouchCancel|UIControlEventTouchUpOutside];
    [self.extraRightButton addTarget:self action:@selector(didTouchDownExtraRightButton:) forControlEvents:UIControlEventTouchDown];
...
- (void)didPressExtraLeftButton:(id)sender {
    self.extraRightButton.enabled = NO;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.extraRightButton.enabled = YES;
    });
...
- (void)didPressExtraRightButton:(id)sender {
    self.extraLeftButton.enabled = NO;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.extraLeftButton.enabled = YES;
    });
...
- (void)didTouchDownExtraRightButton:(id)sender {
    if ([self.delegate respondsToSelector:@selector(tabBarDidTouchDownExtraRightItem:)]) {
        [self.delegate tabBarDidTouchDownExtraRightItem:self];
    }
}
...
- (void)showExtraLeftTabBarItem {
    ...
    [self.extraLeftButton.layer addAnimation:animation forKey:YALExtraLeftBarItemAnimation];
}
...
- (void)showExtraRightTabBarItem {
    ...
    [self.extraRightButton.layer addAnimation:animation forKey:YALExtraRightBarItemAnimation];
}
    
- (void)hideExtraLeftTabBarItem {
    [self.extraLeftButton.layer removeAnimationForKey:YALExtraLeftBarItemAnimation];
...
- (void)hideExtraRightTabBarItem {
    [self.extraRightButton.layer removeAnimationForKey:YALExtraRightBarItemAnimation];
