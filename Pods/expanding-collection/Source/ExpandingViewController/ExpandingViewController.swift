...
public extension ExpandingViewController {
     ...
     */
    func pushToViewController(_ viewController: ExpandingTableViewController, completion: (() -> Void)?) {
        ...
        let stausBarHeight: CGFloat = 0 //insets == true ? UIApplication.shared.statusBarFrame.size.height : 0
        ...
        transitionDriver?.pushTransitionAnimationIndex(currentIndex,
                                                       ...
                                                       insets: tabBarHeight + stausBarHeight, completion:
        { [weak self] headerView in
            headerView.translatesAutoresizingMaskIntoConstraints = false
            headerView.removeConstraints(headerView.constraints)
            
            viewController.headerView.insertSubview(headerView, at: 0)
            headerView.topAnchor.constraint(equalTo: viewController.headerView.topAnchor).isActive = true
            headerView.bottomAnchor.constraint(equalTo: viewController.headerView.bottomAnchor).isActive = true
            headerView.leadingAnchor.constraint(equalTo: viewController.headerView.leadingAnchor).isActive = true
            headerView.trailingAnchor.constraint(equalTo: viewController.headerView.trailingAnchor).isActive = true
...
extension ExpandingViewController {
    ...
    fileprivate func getBackImage(_ viewController: UIViewController, headerHeight: CGFloat) -> UIImage? {
        ...
        let imageFrame = CGRect(origin: CGPoint(x: 0, y: headerHeight), size: imageSize)
