...
open class ImageSlideshowItem: UIScrollView, UIScrollViewDelegate {
    ...
    open var activityIndicator: ActivityIndicatorView?
    ...
    */
    ...
    public required convenience init?(coder aDecoder: NSCoder) {
        if let image = aDecoder.decodeObject(forKey: ImageSlideshowItem.imageKey) as? InputSource {
            self.init(image: image, zoomEnabled: false)
        } else {
            fatalError()
        }
    }
    ...
    open override func encode(with aCoder: NSCoder) {
        ...
        aCoder.encode(image, forKey: ImageSlideshowItem.imageKey)
    }
